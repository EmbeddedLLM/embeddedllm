...
async def benchmark(input_token_length, output_token_length, model_path):
    ...
    model = engine.EmbeddedLLMEngine(model_path, vision=False, device="AMD", backend="directml")
...
for input_token_length in token_ins:
    for output_token_length in token_outs:
        for i in range(50):
            ...
            asyncio.run(benchmark(input_token_length, output_token_length, model_path))

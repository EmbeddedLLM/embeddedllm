...
async def benchmark(input_token_length, output_token_length, model_path, model_name, backend):
    ...
    if backend == "cpu":
        ...
        model = engine.EmbeddedLLMEngine(model_path, vision=False, device="xpu", backend=backend)
    else:
        model = engine.EmbeddedLLMEngine(model_path, vision=False, device="", backend=backend)
